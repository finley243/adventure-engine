...
public class ScriptModulo extends Script {
    ...
    public ScriptModulo(ScriptTraceData traceData, Script firstScript, Script secondScript) {
        super(traceData);
    ...
    ScriptReturnData execute(ScriptRuntime scriptRuntime, Context context) {
        ScriptReturnData firstReturn = firstScript.execute(scriptRuntime, context);
        ...
        ScriptReturnData secondReturn = secondScript.execute(scriptRuntime, context);
        ...
        if (!canModuloExpressions(firstReturn.value(), secondReturn.value())) {
            return new ScriptReturnData(null, null, new ScriptErrorData("Expression received values that could not form a modulo", getTraceData()));
        }
    ...
    private Expression moduloExpressions(Expression firstExpression, Expression secondExpression) {
        if (firstExpression.getDataType() == Expression.DataType.INTEGER && secondExpression.getDataType() == Expression.DataType.INTEGER) {
            ...
            return Expression.integer(value1 % value2);
            ...
            return Expression.decimal(value1 % value2);
        }
        return null;
    }
