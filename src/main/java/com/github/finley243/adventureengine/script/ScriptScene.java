...
public class ScriptScene extends Script {
    ...
    ScriptReturnData execute(ScriptRuntime scriptRuntime, Context context) {
        ...
        context.game().menuManager().sceneMenu(context.game(), context.game().data().getScene(scene.getValueString()), Context.from(context).subject(actor).target(actor).build(), false);
