...
public class Game {
	...
	public Game() throws ParserConfigurationException, SAXException, IOException, GameDataException {
		...
		data().getPlayer().setStatValue("money", Expression.constant(50), this, new Context(data().getPlayer(), data().getPlayer()));
	...
	private void startRound() {
		...
		for (Actor visibleActor : data().getPlayer().getLineOfSightActors()) {
			if (visibleActor.isVisible(data().getPlayer())) {
				visibleActor.triggerScript("on_player_visible_round", new Context(visibleActor, data().getPlayer()));
			}
		}
	...
	private List<Actor> computeTurnOrder() {
		...
		actors.sort((a1, a2) -> a2.getAttribute("agility", new Context(a2, a2)) - a1.getAttribute("agility", new Context(a1, a1)));
		return actors;
	}
