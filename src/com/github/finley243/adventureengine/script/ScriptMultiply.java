...
public class ScriptMultiply extends Script implements ScriptReturnTarget {
    ...
    public void onScriptReturn(RuntimeStack runtimeStack, ScriptReturnData scriptReturnData) {
        if (scriptReturnData.error() != null) {
            ...
            if (canMultiplyExpressions(firstScriptValue, secondScriptValue, runtimeStack.getContext())) {
                Expression multiplyResult = multiplyExpressions(firstScriptValue, secondScriptValue, runtimeStack.getContext());
                runtimeStack.closeContext();
                sendReturn(runtimeStack, new ScriptReturnData(multiplyResult, false, false, null));
            } else {
                runtimeStack.closeContext();
                sendReturn(runtimeStack, new ScriptReturnData(null, false, false, "Expression received values that could not be multiplied"));
            }
        } else {
