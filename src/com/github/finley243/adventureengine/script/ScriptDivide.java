...
public class ScriptDivide extends Script implements ScriptReturnTarget {
    ...
    public void onScriptReturn(RuntimeStack runtimeStack, ScriptReturnData scriptReturnData) {
        if (scriptReturnData.error() != null) {
            ...
            if (canDivideExpressions(firstScriptValue, secondScriptValue, runtimeStack.getContext())) {
                Expression divideResult = divideExpressions(firstScriptValue, secondScriptValue, runtimeStack.getContext());
                runtimeStack.closeContext();
                sendReturn(runtimeStack, new ScriptReturnData(divideResult, false, false, null));
            } else {
                runtimeStack.closeContext();
                sendReturn(runtimeStack, new ScriptReturnData(null, false, false, "Expression received values that could not be divided"));
            }
        } else {
