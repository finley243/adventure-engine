...
public class ActionWeaponReload extends Action {
	...
	public void choose(Actor subject, int repeatActionCount) {
		...
		if (subject == subject.game().data().getPlayer()) {
			if (!ammoType.equals(weapon.getLoadedAmmoType()) && weapon.getAmmoRemaining() > 0) {
				subject.getInventory().addItems(ammoType, weapon.getAmmoRemaining());
				weapon.emptyAmmo();
			}
		...
		Context context = new Context(new MapBuilder<String, Noun>().put("actor", subject).put("weapon", weapon).build());
	...
	public MenuChoice getMenuChoices(Actor subject) {
		return new MenuChoice(LangUtils.titleCase(ammoType.getName()), canChoose(subject), new String[]{"attack", weapon.getName(), "reload (" + weapon.getAmmoRemaining() + "/" + weapon.getClipSize() + ")"}, new String[]{"reload " + weapon.getName() + " with " + LangUtils.pluralizeNoun(ammoType.getName())});
	}
