...
public record ASTFunctionCall(SourceRange nameRange, String name, List<ASTNode> parameters, SourceRange range) implements ASTNode {
    @Override
    public List<HighlightData> highlightData() {
        List<HighlightData> highlights = new ArrayList<>();
        highlights.add(new HighlightData(HighlightType.FUNCTION_CALL, nameRange));
        if (parameters != null) {
